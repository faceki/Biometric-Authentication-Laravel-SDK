...
function showFacekiLoginWindow(type,token){
    url = baseUrl+'/'+newPath+"/faceki-sign-in?route=login&auth="+token;
    window.location.replace(url);
}

function showFacekiPreSignUpWindow(type,token){
    url = baseUrl+'/'+newPath+"/faceki-pre-sign-up";
    window.location.replace(url);
}

function showFacekiSignUpWindow(type,token){
    url = baseUrl+'/'+newPath+"/faceki-sign-up?route=signup&auth="+token;
    window.location.replace(url);
}
